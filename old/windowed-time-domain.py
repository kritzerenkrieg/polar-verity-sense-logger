...
def sliding_window_hrv_analysis(nn_intervals, nn_times, window_duration=30, step_size=1):
    """
    Perform sliding window HRV analysis
    ...
    Returns:
    - DataFrame with HRV metrics for each window
    """
...
def process_single_file(file_path, show_plots=True):
    """Process a single PPG file and return HRV results"""
        ...
        if len(peaks) > 1:
            ...
            if len(nn_intervals_raw) > 0:
                ...
                if len(nn_intervals) > 0:
                    print("Performing sliding window HRV analysis...")
...
def main():
    print("PPG Signal Processing - Sliding Window HRV Analysis")
    print("=" * 60)
