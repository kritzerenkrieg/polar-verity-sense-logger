...
class FeatureStatisticalAnalyzer:
    ...
    def load_data(self):
        ...
        for filename in csv_files:
            pattern = r'pr_([^_]+)_([^_]+)_(truth|lie)-sequence_combined_hrv_v2_svm\.csv'
    ...
    def perform_wilcoxon_analysis(self, df, groupby_column='binary_label'):
        """Perform Wilcoxon rank-sum style analysis for all numeric features grouped by the specified column."""
        ...
        print(f"\nPerforming Wilcoxon rank-sum tests comparing groups: {groups}")
        ...
        for feature in feature_cols:
            ...
            # Perform a two-sided Mann-Whitney U test, which is equivalent to the
            ...
            # Rank-biserial correlation is a better matched effect size here than Cohen's d.
            rank_biserial = (2 * u_statistic) / (len(group_0_values) * len(group_1_values)) - 1
    ...
    def display_results(self, results_df, sort_by='p_value', top_n=20):
        ...
        print(f"\n{'='*100}")
        print(f"STATISTICAL WILCOXON RANK-SUM RESULTS (Top {top_n} features sorted by {sort_by})")
        print(f"Window features excluded from analysis")
        print(f"{'='*100}")
        print(f"{'Feature':<25} {'Group0_Mean':<12} {'Group1_Mean':<12} {'U-Stat':<10} {'P-Value':<12} {'Rank_Biserial':<14} {'Sig':<5}")
        print("-" * 100)
        
        for _, row in results_sorted.iterrows():
            print(f"{row['feature']:<25} {row['group_0_mean']:<12.4f} {row['group_1_mean']:<12.4f} "
                  f"{row['u_statistic']:<10.4f} {row['p_value']:<12.6f} {row['rank_biserial']:<14.4f} {row['significance']:<5}")
    ...
    def analyze_by_domain(self, df):
        ...
        for domain in domains:
            ...
            results = self.perform_wilcoxon_analysis(domain_df)
...
def main():
    ...
    try:
        ...
        results = analyzer.perform_wilcoxon_analysis(df)
        ...
        # Save results to CSV
        if results is not None:
            output_file = os.path.join(data_directory, "wilcoxon_results_no_window.csv")
...
if __name__ == "__main__":
    ...
    if overall_results is not None:
        ...
        top_features = overall_results.sort_values('p_value').head(5)
        for _, row in top_features.iterrows():
            print(f"- {row['feature']}: p={row['p_value']:.6f}, rank-biserial={row['rank_biserial']:.4f}")