...
MODIFIED: Outputs single CSV with combined time and frequency domain features.
"""
...
def sliding_window_hrv_analysis_combined_last40_seconds(nn_intervals, nn_times):
    """
    ...
    Returns combined time and frequency domain features in a single DataFrame
    ...
    Returns:
    - DataFrame with combined HRV metrics for exactly 11 windows (last 40 seconds)
    """
    ...
    for i, start_time in enumerate(window_starts):
        ...
        if len(window_nn) >= 2:  # Need at least 2 NN intervals
            ...
            if time_metrics is not None:
                # Add time domain metrics (excluding duplicates)
                for key, value in time_metrics.items():
                    if key not in combined_metrics:  # Avoid overwriting nn_count
                        combined_metrics[key] = value
        
        # If time domain failed, add NaN values
        ...
        if len(window_nn) >= 10:  # Need at least 10 NN intervals for frequency analysis
            ...
            if freq_metrics is not None:
                # Add frequency domain metrics (excluding duplicates)
                for key, value in freq_metrics.items():
                    if key not in combined_metrics:  # Avoid overwriting existing keys
                        combined_metrics[key] = value
        
        # If frequency domain failed, add NaN values
        if 'lf_power' not in combined_metrics:
            combined_metrics.update({
                'lf_power': np.nan,
                'hf_power': np.nan,
                'lf_hf_ratio': np.nan,
        ...
        if len(peaks) > 1:
            ...
            if len(nn_intervals_raw) > 0:
                ...
                if len(nn_intervals) > 0:
                    print("Performing combined sliding window HRV analysis on LAST 40 SECONDS (11 windows)...")
                    
                    # Perform combined sliding window analysis - LAST 40 SECONDS ONLY
                    combined_results = sliding_window_hrv_analysis_combined_last40_seconds(
                        nn_intervals, nn_times
                    )
                    
                    if not combined_results.empty:
                        print(f"Successfully analyzed {len(combined_results)} windows with {len(combined_results.columns)} combined features")
                        ...
                        if show_plots:
                            create_combined_plots(combined_results, time, ppg_filtered, peaks, nn_intervals, nn_times, metadata)
...
def create_combined_plots(combined_results, time, ppg_filtered, peaks, nn_intervals, nn_times, metadata):
    """Create comprehensive plots for combined analysis"""
    
    fig = plt.figure(figsize=(20, 20))
    fig.suptitle(f'Combined HRV Analysis - {metadata["subject"]} {metadata["condition"]} {metadata["label"]}', fontsize=16, fontweight='bold')
    ...
    # Plot 3: SDNN and RMSSD
    ax3 = plt.subplot(5, 2, 3)
    valid_sdnn = combined_results.dropna(subset=['sdnn'])
    valid_rmssd = combined_results.dropna(subset=['rmssd'])
    if not valid_sdnn.empty:
        plt.plot(valid_sdnn['window_start'], valid_sdnn['sdnn'], 'b-o', markersize=3, linewidth=1.5, label='SDNN')
    if not valid_rmssd.empty:
        plt.plot(valid_rmssd['window_start'], valid_rmssd['rmssd'], 'r-o', markersize=3, linewidth=1.5, label='RMSSD')
    plt.title('Time Domain: SDNN & RMSSD')
    plt.ylabel('Value (ms)')
    plt.xlabel('Time (s)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Plot 4: pNN20 and pNN50
    ax4 = plt.subplot(5, 2, 4)
    valid_pnn20 = combined_results.dropna(subset=['pnn20'])
    valid_pnn50 = combined_results.dropna(subset=['pnn50'])
    ...
    plt.title('Time Domain: pNN20 & pNN50')
    plt.ylabel('Percentage (%)')
    plt.xlabel('Time (s)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Plot 5: LF and HF Power
    ax5 = plt.subplot(5, 2, 5)
    valid_lf = combined_results.dropna(subset=['lf_power'])
    valid_hf = combined_results.dropna(subset=['hf_power'])
    if not valid_lf.empty:
        plt.plot(valid_lf['window_start'], valid_lf['lf_power'], 'b-o', markersize=3, linewidth=1.5, label='LF Power')
    if not valid_hf.empty:
        plt.plot(valid_hf['window_start'], valid_hf['hf_power'], 'r-o', markersize=3, linewidth=1.5, label='HF Power')
    plt.title('Frequency Domain: LF & HF Power')
    plt.ylabel('Power (ms²)')
    plt.xlabel('Time (s)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    valid_ratio = combined_results.dropna(subset=['lf_hf_ratio'])
    ...
    plt.title('Frequency Domain: LF/HF Ratio')
    ...
    valid_hr = combined_results.dropna(subset=['nn_mean'])
    ...
    # Plot 8: Normalized Powers
    ax8 = plt.subplot(5, 2, 8)
    valid_lf_norm = combined_results.dropna(subset=['lf_norm'])
    valid_hf_norm = combined_results.dropna(subset=['hf_norm'])
    ...
    plt.title('Normalized LF & HF Powers')
    plt.ylabel('Normalized Power (%)')
    plt.xlabel('Time (s)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Plot 9: SDSD and Triangular Index
    ax9 = plt.subplot(5, 2, 9)
    valid_sdsd = combined_results.dropna(subset=['sdsd'])
    if not valid_sdsd.empty:
        plt.plot(valid_sdsd['window_start'], valid_sdsd['sdsd'], 'cyan', marker='d', markersize=3, linewidth=1.5)
    plt.title('Time Domain: SDSD')
    plt.ylabel('SDSD (ms)')
    plt.xlabel('Time (s)')
    plt.grid(True, alpha=0.3)
    
    # Plot 10: Natural Log of HF Power
    ax10 = plt.subplot(5, 2, 10)
    valid_ln_hf = combined_results.dropna(subset=['ln_hf'])
    if not valid_ln_hf.empty:
        plt.plot(valid_ln_hf['window_start'], valid_ln_hf['ln_hf'], 'g-o', markersize=3, linewidth=1.5)
    plt.title('Frequency Domain: ln(HF)')
...
def display_combined_summary_statistics(combined_results):
    """Display summary statistics for combined results"""
    print("\n" + "="*80)
    print("SUMMARY STATISTICS ACROSS ALL WINDOWS - COMBINED FEATURES")
    print("="*80)
...
def main():
    print("PPG Signal Processing - Combined HRV Analysis (Last 40 Seconds Only)")
    print("30-second windows moving 1 second forward = 11 windows per file")
    print("Generates COMBINED time-domain and frequency-domain features in single CSV")
    print("=" * 80)
    ...
    print(f"\nOutput CSVs will be written to: {current_dir}")
    ...
    if choice == 0:
        ...
        if combined_results is not None:
            ...
            output_filename = os.path.join(current_dir, f"{base_name}_combined_hrv.csv")
            combined_results.to_csv(output_filename, index=False)
            print(f"\nCombined HRV results saved to: {output_filename}")
            print(f"Output contains exactly {len(combined_results)} samples (last 40 seconds)")
        ...
        for i, file_path in enumerate(txt_files):
            ...
            if combined_results is not None:
                ...
                print(f"  Combined: {os.path.relpath(output_filename, current_dir)} ({len(combined_results)} samples, {len(combined_results.columns)} features)")
                successful_files += 1
            else:
        ...
        print(f"Each successful file contains exactly 11 samples (last 40 seconds)")
        print(f"Combined time-domain and frequency-domain features in single CSV")
