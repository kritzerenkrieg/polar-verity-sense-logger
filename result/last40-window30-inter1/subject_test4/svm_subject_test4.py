...
- ENHANCED: Detailed logging of all feature-parameter combinations
"""
...
class BalancedPerSubjectBoundaryTrainer:
    def __init__(self, data_directory='.', min_features=2, max_features=3, 
        ...
        # NEW: Storage for detailed combination logging
        self.detailed_combination_results = []
    ...
    def log_combination_result(self, subject_id, feature_combo, config, metrics, 
                              n_samples, class_dist, cv_folds, cv_std):
        """NEW: Log detailed combination result for CSV output."""
        # Create feature combination string (sorted for consistency)
        feature_combo_str = '|'.join(sorted(feature_combo))
        
        # Create config string
        config_str = f"{config['kernel']}_C{config['C']}"
        if 'gamma' in config:
            config_str += f"_gamma{config['gamma']}"
        ...
        self.detailed_combination_results.append(result_entry)
            ...
            for config in self.config_space:
                ...
                try:
                    ...
                    if cv_scores:
                        ...
                        # Calculate full metrics for logging
                        # Re-evaluate on full data for complete metrics
                        scaler_full = StandardScaler()
                        X_scaled_full = scaler_full.fit_transform(X_combo)
                        svm_full = SVC(**config, random_state=42)
                        svm_full.fit(X_scaled_full, y_combo)
                        y_pred_full = svm_full.predict(X_scaled_full)
                        full_metrics = self.evaluate_with_balanced_metrics(y_combo, y_pred_full)
                        
                        # NEW: Log this combination result
                        self.log_combination_result(
                            subject_id=subject_id,
                            feature_combo=combo,
                            config=config,
                            metrics=full_metrics,
                            n_samples=len(X_combo),
                            class_dist=f"{np.sum(y_combo == 0)}/{np.sum(y_combo == 1)}",
    ...
    def save_balanced_results(self, subject_models, cross_results, generalization_scores, best_subject):
        ...
        # NEW: Save detailed combination results
        if self.detailed_combination_results:
            detailed_df = pd.DataFrame(self.detailed_combination_results)
            
            # Sort by combination for easy analysis
            ...
            detailed_df.to_csv('detailed_feature_parameter_combinations.csv', index=False)
            print("✓ 'detailed_feature_parameter_combinations.csv' - ALL combinations tested")
            
            # Create summary statistics by combination
            combo_summary = detailed_df.groupby('feature_combination').agg({
                'accuracy': ['mean', 'std', 'min', 'max', 'count'],
                'balanced_score': ['mean', 'std', 'min', 'max'],
                'f1_score': ['mean', 'std'],
                'n_features': 'first',
                'features_list': 'first'
            }).round(4)
            ...
            # Sort by mean balanced score
            combo_summary = combo_summary.sort_values('balanced_score_mean', ascending=False)
            
            combo_summary.to_csv('feature_combination_summary.csv', index=False)
            print("✓ 'feature_combination_summary.csv' - Summary statistics per combination")
        ...
        summary = {
            ...
            'use_regularization': self.use_regularization,
            'total_combinations_tested': len(self.detailed_combination_results),
            'timestamp': datetime.now().isoformat()
        }
...
def main():
    ...
    print("• ENHANCED: Detailed logging of all combinations")
    ...
    try:
        ...
        if results:
            ...
            # NEW: Show combination analysis
            if trainer.detailed_combination_results:
                total_combinations = len(trainer.detailed_combination_results)
                unique_combos = len(set(r['feature_combination'] for r in trainer.detailed_combination_results))
                print(f"\n📊 COMBINATION ANALYSIS:")
                print(f"   Total tests run: {total_combinations}")
                print(f"   Unique feature combinations: {unique_combos}")
                print(f"   Average tests per subject: {total_combinations / len(trainer.all_data['subject'].unique()):.1f}")
            
            print(f"\n📁 Enhanced output files:")
            print("• detailed_feature_parameter_combinations.csv - ALL combinations tested per subject")
            print("• feature_combination_summary.csv - Summary statistics per feature combination")
            ...
            print(f"\n🔍 ANALYSIS RECOMMENDATIONS:")
            print("1. Check 'detailed_feature_parameter_combinations.csv' to see which combinations work best")
            print("2. Use 'feature_combination_summary.csv' to identify consistently good feature sets")
            print("3. Look for patterns in subject-specific vs. generalizable combinations")
            print("4. Compare kernel performance (linear vs RBF) across subjects")
